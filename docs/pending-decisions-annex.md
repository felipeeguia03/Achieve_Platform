...
| C01-019 | TopicProgress y resumen de materia | Product Progress; Hoy/Materia/Bitácora/Risk; C01-003/018 | 🟡 **`ANSWERED — RESIDUO ABIERTO`** — 5 de septiembre de 2026 ([ADR-059](decisions.md#adr-059)). Se conserva lo actual: `UX06` distingue un cambio de un «sin cambio confirmado» y `UX02` **omite** las dimensiones sin semántica aprobada. **No bloquea el MVP.** Residuo: nombres observables y escalas, para validar con evidencia del piloto — y con él **el gate `H` sigue en pie** para llevar `UX06` a high-fidelity | P1 | H |
...
| C01-021 | Risk Engine v1 y sujeto de RiskSignal | Risk owner; TodayView/CRM/ADE; C01-001/002/019/023/036 | 🟡 **`ANSWERED — RESIDUO ABIERTO`** — 5 de septiembre de 2026 ([ADR-055](decisions.md#adr-055)). El sujeto quedó resuelto por schema y ✅ `HP0-06-1 v4.0-psicopedagogia` evalúa con el denominador y las condiciones de [ADR-037](decisions.md#adr-037); sus umbrales `2` y `3` viven en configuración. **`HP0-06-2` y `HP0-06-3` permanecen en modo `HUMANA` hasta disponer de evidencia del piloto**, y sus umbrales se definirán con datos observados ahí. **No autoriza evaluadores automáticos ni umbrales provisionales**, y el guard estático que lo impide sigue en pie | P1 | I |
...
| C01-029 | Readiness scoped de ExamPreparation | Product; UX08/ADE/Risk; C01-025–028/031–038 | ✅ **`CLOSED`** — 5 de septiembre de 2026 ([ADR-058](decisions.md#adr-058)). **Sin porcentaje y sin predicción de aprobación:** `READY_BY_PROTOCOL` significa que cumplió el protocolo. Regla determinística sobre los tres estados canónicos, con `explanation` obligatoria en toda escritura. ⚠️ **La tabla sigue sin escritor** —implementarla es trabajo con su propia etapa— y queda un residuo técnico: qué valores admite `evidence_status`, hoy `text` sin `CHECK` | P1 | I |
| C01-030 | Autorización, permisos y privacidad institucional | Product Security/Privacy; todas las superficies/CRM; C01-001/002/017/039 | ⏸️ **`DEFERRED` con motivo** — 5 de septiembre de 2026 ([ADR-057](decisions.md#adr-057)). La identidad permanente **se difiere hasta el cierre de [ADR-006](decisions.md#adr-006)**: ninguna de las tres operaciones puede tocar a una persona real antes, así que elegir ahora sería elegir sin el dato que lo decide. Se mantiene el interinato —identidad de servicio, proceso identificado en el evento, **sin fabricar UUID de personas inexistentes**—. **No autoriza revisar, corroborar ni pedir reenvíos sobre evidencia de estudiantes reales** | P1 | I |
...
| C01-036 | `HUMAN-P0-06`: aplicabilidad de revisión humana | Psicopedagoga real; Protocol/Validation; **y Risk/Intervention** | **ANSWERED 31 ago 2026** — selectiva y proporcional; **la persona entra por la situación del estudiante, no por el tipo de entrega**. Residuo: **umbral de "error reiterativo"**. 🟡 **Umbral provisional cargado** por [ADR-036](decisions.md#adr-036): dos apariciones corroboradas del mismo `error_type`, alcance preparación, reinicio con resolución limpia. ⚠️ **`PROVISIONAL — REQUIRES POST-MVP HUMAN VALIDATION`, autoridad Product Owner. Sigue OPEN**: ella lo valida antes del piloto. ✅ **RESPONDIDA el 2 de septiembre de 2026** por la psicopedagoga ([ADR-037](decisions.md#adr-037), fuente en [`validacion-psicopedagogica-source.md`](validacion-psicopedagogica-source.md)): 6 `CAMBIAR` + 1 `APROBAR`. **Los umbrales quedaron donde estaban**; cambió la unidad de conteo. **Sigue OPEN para datos reales**: condicionada a piloto, revisión humana, explicabilidad, accesibilidad y monitoreo de equidad | P1 | I |
...
| C01-040 | Webhooks, sincronización y reconciliación | Integration/Security; CRM/Evidence/Event Model; C01-015/023/030/039 | OPEN; retry de autorización v1 definido, webhooks/reconciliación futuros sin contrato. Alcanza a los tres flujos de [`contrato-riesgo-candidato-v0.2.md`](contrato-riesgo-candidato-v0.2.md), cuyo **diseño el CRM ya aceptó sin objeciones**. ⏸️ **Diferido por [ADR-035](decisions.md#adr-035)** al final del Track B: faltan tres definiciones de forma (envelope de error, `cause.code`, secretos con rotación) y construcción, casi toda del lado del CRM | P1 | I |
...
| C01-044 | Playbooks, SLA y Human QA del piloto | Product Operations; Intervention/CRM; C01-022/039/046 | 🟡 **`ANSWERED — RESIDUO ABIERTO`** — 5 de septiembre de 2026 ([ADR-056](decisions.md#adr-056)). **Playbook y SLA provisionales y rotulados**: revisar contexto, primer contacto por WhatsApp, identificar bloqueo, acordar una única próxima acción o activar rescate/no cortar, y registrar outcome y próximo seguimiento; primer contacto **dentro de cuatro horas hábiles** del horario operativo, y si la señal cae fuera, el plazo empieza en la siguiente ventana. ⚠️ **No define la identidad de la persona ni autoriza superficies de operador en Plataforma.** ⬜ Sigue abierto **de qué sistema es el playbook** —la propuesta de que sea canónico del CRM no la cierra este ADR— y **no existe tabla de ventana operativa**: las cuatro horas son texto del playbook, no un `sla_at` calculado | P1 | P |
