...
catálogo, sólo lo del estudiante.

**Ahora hay dos estudiantes sintéticos**, y son dos recorridos distintos:

| Credenciales | Quién es |
|---|---|
| `estudiante.sintetico@achieve.local` / `achieve-demo-sintetica` | **Con el alta completa.** Entra directo a `HOY` y recorre el loop de abajo |
| `estudiante.nuevo@achieve.local` / `achieve-demo-alta` | **Recién habilitado.** Sin consentimiento, sin carrera y sin materias: recorre el alta |
...
## El alta, primero de todo — Fase B6.14

**Esto no estaba hasta el 5 de septiembre**, y era el hueco más grande del producto:
[ADR-039](decisions.md#adr-039) lo dejó escrito —*"entre el `authorized: true` del CRM y la primera
acción del estudiante no hay ninguna pantalla definida"*— y [ADR-042](decisions.md#adr-042) decidió
el orden entero. Ahora se recorre.

**Se entra con el segundo estudiante:** `estudiante.nuevo@achieve.local` / `achieve-demo-alta`.
...
⚠️ **Los planes de la UCC están cargados y en `DRAFT`**, así que no aparecen en el selector:
`Universidad SYN (sintética)` e `Instituto SYN-2 (sintético)` son las dos que sí. Por qué, en
[ADR-053](decisions.md#adr-053) y en [`catalogo/README.md`](../catalogo/README.md).

```bash
# Qué hay cargado, y en qué estado
docker exec supabase_db_achieve-platform psql -U postgres -d postgres -c "
  select i.key, ap.key as carrera, cp.version, cp.publication_status,
         count(cr.*) as requisitos, count(cr.course_id) as materias
    from institution i join academic_program ap on ap.institution_id=i.id
    join curriculum_plan cp on cp.program_id=ap.id
    left join curriculum_requirement cr on cr.curriculum_plan_id=cp.id
   group by 1,2,3,4 order by 1,2;"
```

---

## El loop diario — Fase B6.8
