...
export function proyectarHoy(escenario: Escenario): HoyProps | null {
  ...
  return {
    ...
    recuperacion: null,
    /**
     * **Siempre `null` en el Track A** — [ADR-093](../../docs/decisions.md#adr-093).
     *
     * El tablero sale de `GET /api/tablero`, y un escenario no tiene backend.
     * Dibujarlo desde el fixture obligaría a declarar una segunda vez las
     * evaluaciones y la cobertura que `UX02_INDICE` ya proyecta, y **dos
     * declaraciones del mismo dato son dos datos**.
     */
    tablero: null,
