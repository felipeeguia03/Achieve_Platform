...
/**
 * 🧪 LABORATORIO DESCARTABLE — un diálogo modal accesible, sin librería.
 *
 * Al abrir, el foco va al primer botón; Tab queda adentro; Escape cierra; al
 * cerrar, el foco vuelve a lo que lo abrió. No se usa `<dialog>` porque jsdom no
...
export function Dialogo({
  ...
  useEffect(() => {
    ...
    const enfocables = () =>
      Array.from(caja.current?.querySelectorAll<HTMLElement>("button:not([disabled]), [href], [tabindex]:not([tabindex='-1'])") ?? []);
    enfocables()[0]?.focus();
