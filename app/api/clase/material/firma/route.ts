...
import { firmarMaterialDeClase, resolverSesion } from "@/lib/server/composicion";

/**
 * `/api/clase/material/firma` — la URL firmada para subir un archivo de la clase ·
 * [ADR-099](../../../../../docs/decisions.md#adr-099) §5.
 *
 * La clave del objeto **la deriva el servidor**. Lo declarado (tipo y tamaño) se
 * vuelve a comprobar contra el storage al registrar.
 */
...
/** `POST { clase, nombre, mime, bytes }` → `{ clave, url, token }`. */
export async function POST(request: Request) {
  ...
  const r = await firmarMaterialDeClase(s.estudiante.institutionId, {
