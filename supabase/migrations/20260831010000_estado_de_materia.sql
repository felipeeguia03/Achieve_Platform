-- ─────────────────────────────────────────────────────────────────────────────
-- Achieve Platform · Etapa B2.6 — `UX02` en una sola lectura
--
-- Mismo argumento que `estado_del_dia`: la materia mira cursado, Action,
-- Commitment, Evidence, unidades y progreso a la vez, y varias lecturas dan una
-- foto inconsistente entre sí.
--
-- ## Lo que esta función NO devuelve, y por qué
--
-- **Ningún valor numérico de `topic_progress`.** `VI.2` §8.6 es explícito: *«si
-- no existe semántica aprobada para mostrar una dimensión, omite la síntesis o
-- muestra un hecho comprensible; **nunca expone un valor interno bruto**»*. Las
-- columnas `*_value` son `NUMERIC` sin unidad ni escala —eso es `C01-019`, gate
-- `H`, `OPEN`—, así que se devuelve **el estado de cada dimensión**, que sí es
-- un hecho cerrado, y nunca el número.
--
-- **Ninguna brecha derivada.** `VI.2` §8.4 la condiciona: *«sólo aparece si
-- Student/Risk Model entrega la brecha derivada y su explicación; **la vista no
-- compara umbrales**»*. `C01-043` (Student Model) está `OPEN`, así que
-- confianza y dominio salen como dos hechos separados y nadie los compara.
--
-- **Ningún estado de materia.** Ver `20260831000000_estado_sin_afirmacion.sql`.
-- ─────────────────────────────────────────────────────────────────────────────
