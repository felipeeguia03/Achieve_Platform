-- Achieve Platform · Fase B6.24 — el peso declarado entra por la ingesta.
--
-- ADR-086. `topic.weight` existe desde la B6.19
-- (`20260920000000_duracion_y_tipo_de_clase.sql`) y **ninguna ruta lo escribía**:
-- `ingerir_materia` leía `codigo`, `nombre` y `orden` de cada unidad, y nada más.
-- El resultado era que `lib/domain/duracion.ts` trataba a **todas** las materias
-- como sin pesos y el reparto de minutos por tema salía parejo siempre.
--
-- ⚠️ **El peso NO es la dificultad, y no se renombra a eso.** Es cuánto de la
-- materia ocupa esa unidad, y de ahí salen los minutos. Que una unidad larga sea
-- además difícil es otra afirmación, y nadie la declaró.
--
-- ⚠️ **Es todo o nada por materia** (`usaPesos()`, ADR-068): si una unidad tiene
-- peso y otra no, la materia entera vuelve a repartirse pareja. Por eso quien
-- llame a esta función manda el peso de **todas** sus unidades o de ninguna.
-- Un peso faltante no es `1.0`: es ausencia de dato, y mezclarlos produciría un
-- reparto que **parece medido y no lo es**.
--
-- Sólo cambia el `INSERT INTO topic`. El resto de la función queda igual.
...
BEGIN
  ...
  FOR r IN SELECT * FROM jsonb_array_elements(p_unidades) LOOP
    INSERT INTO topic (offering_id, code, name, sequence, weight)
    VALUES (v_offering, r->>'codigo', r->>'nombre', (r->>'orden')::INTEGER,
            -- ⚠️ **Ausente es `NULL`, no `1.0`.** `->>'peso'` de una clave que no
            -- vino devuelve NULL y el cast lo conserva: es exactamente lo que
            -- `duracion.ts` espera para tratar la materia como sin pesos.
            (r->>'peso')::NUMERIC)
