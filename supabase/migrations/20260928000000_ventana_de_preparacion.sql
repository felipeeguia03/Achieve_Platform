-- ─────────────────────────────────────────────────────────────────────────────
-- Achieve Platform · La punta izquierda de la ventana de preparación
--
-- Ejecuta [ADR-078](../../docs/decisions.md#adr-078): el «Gantt del período».
--
-- ## Un solo hecho más
--
-- La ventana de preparación es `[primera clase dictada , fecha de evaluación]`,
-- y la punta derecha ya viajaba desde
-- [ADR-077](../../docs/decisions.md#adr-077). Falta la izquierda.
--
-- ⚠️ **Se descartó empezar todas las barras en el borde del eje**, que es como
-- se ven en el mockup. Es cómodo y **no afirma nada**: una materia que empezó en
-- marzo y una que empezó la semana pasada tendrían la misma ventana.
--
-- ⚠️ **Y sigue sin decidir nada.** El eje, el recorte y el relleno los calcula
-- `lib/domain/ventana.ts` y `lib/domain/cobertura.ts`. Acá viaja un `MIN`.
-- ─────────────────────────────────────────────────────────────────────────────
...
COMMENT ON FUNCTION public.insumos_de_reparto IS
  'ADR-073 corte 2, extendida por ADR-077 y ADR-078. Entrega HECHOS por materia; el reparto lo '
  'calcula lib/domain/reparto.ts, la cobertura lib/domain/cobertura.ts y la ventana '
  'lib/domain/ventana.ts. minutosPorSemana NULL = no contestó la pregunta, que no es lo mismo que '
  'cero. evaluacion NULL = no hay evaluación con fecha futura, y NO se cae a una pasada. '
  'primeraClase NULL = no hay clases cargadas, que no es lo mismo que empezar hoy.';
