-- ─────────────────────────────────────────────────────────────────────────────
-- ADR-095 · El aula viaja con el bloque horario hasta `UX02` y el índice.
--
-- [ADR-094](../../docs/decisions.md#adr-094) le dio la columna a
-- `class_schedule_block`; esta función todavía no la devolvía, así que «Clases
-- de la semana» mostraba el horario sin el lugar.
--
-- ⚠️ **Un solo cambio**: `'aula', b.room` dentro del `horario`. El resto del
-- cuerpo es **el vigente, volcado con `pg_get_functiondef`** — no se retipeó, que
-- es como se pierde una cláusula sin que nadie lo note.
--
-- ⚠️ **El aula no trae procedencia propia**: viaja al lado de `fuente` y
-- `verificacion` del bloque, que son las suyas.
-- ─────────────────────────────────────────────────────────────────────────────
