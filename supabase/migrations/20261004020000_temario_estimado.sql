-- Achieve Platform · Fase B6.24 — la materia dice qué parte de su contenido
-- la declaró la cátedra y qué parte la estimó Achieve.
--
-- ADR-086. Con los temarios reales de la UCC cargados y los que faltan
-- generados, en la misma pantalla conviven dos cosas muy distintas: unidades que
-- salieron del programa oficial y unidades que **inventó el sistema**. Mostrar
-- las dos igual sería `A-01` — *dato roto presentado como transparencia*.
--
-- ⚠️ **Esto NO es `verification_status` y no lo reemplaza.** Son dos preguntas:
-- «¿de dónde salió?» la contesta `source_type`, y «¿alguien con autoridad lo
-- confirmó?» la contesta `verification_status`, cuya única escritura sigue
-- siendo `corroborar_procedencia()` (`I9`). Todo lo generado queda `unverified`.
--
-- Sólo se agrega la clave `contenido`. El resto de la función queda igual.
